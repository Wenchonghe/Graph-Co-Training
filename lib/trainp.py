...
class gnn():
    ...
    def fit(self, train_data, infer_labels,  labels, train_idx, val_idx, L):
        t_process, t_wall = time.clock(), time.time()
        sess = tf.Session(graph=self.graph)
        ...
        for step in range(1, num_steps+1):
            ...
            feed_dict = {self.ph_data: batch_data, self.ph_labels: batch_labels, self.ph_dropout: self.dropout,self.is_training: 1,
                        self.Lsp0_indices: [[i, j] for i, j in zip(*L[idx].nonzero())],  self.Lsp0_val: L[idx].data, self.Lsp0_shape: L[idx].shape,}
...
#                 print('  learning_rate = {:.2e}, loss_average = {:.2e}'.format(learning_rate, loss_average))
                ...
                for i in range(val_data.shape[0]):
                    ...
                    feed_dict = {self.ph_data:batch_valdata ,self.ph_labels:batch_vallabel , self.is_training: 0,
                        ...
                        self.Lsp0_indices: [[i, j] for i, j in zip(*val_L.nonzero())],  self.Lsp0_val: val_L.data, self.Lsp0_shape: val_L.shape,}
...
#                 summary.ParseFromString(sess.run(self.op_summary, feed_dict))
    ...
    def build_graph(self, M_0, F_0,F_1):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope('inputs'):
                ...
                self.Lsp0_indices = tf.placeholder(tf.int64)
                self.Lsp0_shape = tf.placeholder(tf.int64)
                self.Lsp0_val = tf.placeholder(tf.float32)
                self.L = tf.SparseTensor(self.Lsp0_indices, self.Lsp0_val, self.Lsp0_shape)
                
                self.val_truelabels = tf.placeholder(tf.int64, (self.batch_size, None, F_1), 'val_truelabels')

            # Model.
            op_outputs = self.inference(self.ph_data, self.ph_dropout, self.L, self.is_training)
    ...
    def chebyshev5(self, x, L, Fout, K):
        ...
        for k in range(2, K):
            ...
            x = concat(x, x2)
            x0, x1 = x1, x2
        x = tf.reshape(x, [K, -1, Fin, N])  # K x M x Fin x N
    ...
    def call(self, x, dropout, L, is_training, reuse=False):
        rate = 0.2
        with tf.variable_scope('gnn1', reuse=reuse):
            N, Min, Fin = x.get_shape()
            with tf.name_scope('filter1'):
                x = self.filter(x, L, self.F, self.K)
                print(L, self.F, self.K)
        ...
        with tf.variable_scope('gnn2', reuse=reuse):
            with tf.name_scope('filter2'):
                x = self.filter(x, self.L, self.classnum, self.K)
                print(self.L, self.classnum, self.K)
    ...
    def inference(self, x, dropout, L,is_training ):
        x = self.call(x,dropout, L, is_training )
